  ...
  template: `
    <div class="w-full max-w-7xl mx-auto p-8 md:p-12 flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 text-foreground">
      <div class="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 border-b border-border pb-6">
        <div>
          <h1 class="text-5xl md:text-6xl font-display tracking-widest uppercase text-foreground drop-shadow-md">Recetas de Menú</h1>
          <p class="text-muted-foreground mt-2 text-lg">Asigna ingredientes a cada combo para descontar stock automáticamente al vender.</p>
        </div>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-border bg-muted/30">
                <th class="font-display tracking-widest uppercase text-muted-foreground text-sm py-4 px-4">Producto del Menú</th>
                <th class="font-display tracking-widest uppercase text-muted-foreground text-sm py-4 px-4">Ingredientes Necesarios</th>
            ...
            <tbody>
              @if (loading()) {
                <tr>
                  <td colspan="3" class="py-16 text-center text-muted-foreground animate-pulse">
                ...
                <tr>
                  <td colspan="3" class="py-20 text-center text-muted-foreground">
                ...
                @for (item of recetas(); track item._id || item.id) {
                  <tr class="hover:bg-muted/30 transition-colors border-b border-border group">
                    ...
                    <td class="p-4 text-center">
                      <button (click)="editar(item)" class="text-neutral-500 hover:text-blue-500 hover:bg-blue-500/10 p-2 rounded transition-colors mr-2" title="Editar receta">
                      ...
                      <button (click)="eliminar(item._id || item.id)" class="text-neutral-500 hover:text-red-500 hover:bg-red-500/10 p-2 rounded transition-colors" title="Eliminar receta">
