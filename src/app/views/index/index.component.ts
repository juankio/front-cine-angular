  ...
  template: `
    <app-default-layout>
      <div class="max-w-7xl mx-auto px-4 md:px-6 py-10 flex flex-col gap-10">
        ...
        <hlm-card class="relative flex flex-col p-8 md:p-14 w-full shadow-sm border-border bg-card overflow-hidden">
          ...
          <div class="relative z-10 flex flex-col md:flex-row gap-8 w-full items-center justify-between">
            ...
            <div class="relative w-full md:w-96 flex-shrink-0">
              ...
              <input hlmInput type="text" placeholder="Buscar película..." [value]="searchTerm()" (input)="onSearchInput($event)" class="w-full pl-12 h-14 text-lg rounded-xl bg-background border-border shadow-sm focus-visible:ring-primary relative z-10 transition-all hover:border-primary/50" />
            </div>
          </div>
        </hlm-card>

        @if (!searchTerm()) {
          <!-- En vivo hoy (Oculto al buscar) -->
          ...
          <!-- Combos para hoy (Oculto al buscar) -->
          <div class="animate-in fade-in duration-500 delay-100 fill-mode-both">
            <div class="flex items-center gap-2 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-primary"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
        ...
        <!-- En cartelera / Resultados -->
        <div class="mb-6 animate-in fade-in duration-300">
          <div class="flex items-center gap-2 mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-primary"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
            <p class="text-lg font-semibold uppercase tracking-wider font-display">
              {{ searchTerm() ? 'Resultados de Búsqueda' : 'En cartelera' }}
            </p>
          </div>
          
          @if (peliculas().length > 0) {
            @if (peliculasFiltradas().length > 0) {
              <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-in fade-in duration-300">
                @for (pelicula of peliculasFiltradas(); track pelicula._id || pelicula.id) {
                  <app-peli-cartelera
              ...
              <div class="flex flex-col p-10 w-full items-center justify-center text-center border border-border rounded-xl shadow-sm bg-card/50 backdrop-blur-xl min-h-[200px] animate-in fade-in duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground/30 mb-4"><circle cx="11" cy="11" r="8"/><line x1="21" x2="16.65" y1="21" y2="16.65"/></svg>
                <p class="text-xl font-medium font-display uppercase tracking-widest text-muted-foreground">Sin resultados</p>
                <p class="text-muted-foreground mt-2 text-sm">No encontramos películas con el título "{{ searchTerm() }}".</p>
              </div>
            }
          } @else {
