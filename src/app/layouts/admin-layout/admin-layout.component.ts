...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-background text-foreground relative flex flex-col overflow-hidden">
      <!-- Fondo Minimalista -->
      <div class="absolute inset-0 z-0 pointer-events-none">
        <div class="absolute top-0 inset-x-0 h-full bg-gradient-to-b from-primary/5 via-transparent to-transparent"></div>
      </div>

      <div class="relative z-10 border-b border-border/50">
      ...
      <div class="relative z-10 flex flex-col md:flex-row w-full h-[calc(100vh-64px)] overflow-hidden">
        ...
        <aside class="hidden md:flex w-64 shrink-0 border-r border-border/50 bg-card/40 backdrop-blur-xl flex-col z-0 relative">
          ...
          <ul class="flex flex-col gap-1 px-3 py-4 overflow-y-auto">
            @for (item of navItems; track item.to) {
              <li>
                <a
                  ...
                  class="flex items-center gap-3 px-4 py-2.5 w-full justify-start rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors group"
                >
